...
def get_n_agg_pages(headers):
    ...
    # with open('data_collection/functions/sitemap/sitemap.xml', 'r') as f:
    #     data = f.read()
    ...
    # n_agg_pages = len(os.listdir('data_collection/functions/sitemap/'))-1
    return n_agg_pages
...
for i in range(1,n_agg_pages+1): # number of content-aggregating pages from https://cointelegraph.com/sitemap.xml
    url = url_base+str(i)
    print('scrapping ', url)
    web_map = scraper.get(url, headers = headers)
    print("Scraping page", i, "of", n_agg_pages, "pages")
    # web_map = open('data_collection/functions/sitemap/post-sitemap-'+str(i)+'.xml', 'r')
    ...
    for j, item in enumerate(all_links):
        ...
        page = scraper.get(url_post, headers = headers)
        page.encoding = 'utf-8'
